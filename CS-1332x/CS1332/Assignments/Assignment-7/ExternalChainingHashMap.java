...
public class ExternalChainingHashMap<K, V> {
    ...
    public V put(K key, V value) {
      ...
      int index = comprision(key, table.length);
      ExternalChainingMapEntry<K, V> data = table[index];
      if (data != null){
        ...
        if (key.equals(data.getKey())){
          V temp = data.getValue();
          data.setValue(value);
          return temp;
        }
        else{
          ExternalChainingMapEntry<K, V> t = new ExternalChainingMapEntry<K, V>(key, value);
          t.setNext(table[index]);
          table[index] = t;
          size++;
        }
      }
    ...
    private int comprision(K key, int length){
    ...
    private void resizeBackingTable(int length) {
      ...
      ExternalChainingMapEntry<K, V>[] temp = (ExternalChainingMapEntry<K, V>[]) new ExternalChainingMapEntry[length];
        ...
        for (ExternalChainingMapEntry<K, V> data : table){
          if(data != null){
            int index = comprision(data.getKey(), length);
            temp[index] = data;
          }
